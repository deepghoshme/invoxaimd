...
Save screenshots to `.qa-out/<name>.png` and write a run summary to
`.qa-out/results.json` (schema: `{ranAt, total, passed, failed, targets:[{name,url,status,ok,screenshot,consoleErrors,pageErrors,failedResponses}]}`).
